...
import RadioButton from '../../1-atoms/RadioButton/radio-button.component';
import Label from '../../1-atoms/Label/label.component'; 
import { validateGameName, validatePlayerNameExists } from '../../../utils/validators';
import './join-game-form.component.scss';
import { Player } from '../../../hooks/useRoomState';

export type PlayerRole = 'player' | 'spectator';

interface JoinGameFormProps {
  onJoinSuccess: (name: string, role: PlayerRole) => void;
  currentPlayers: Player[]; // Add this prop to receive existing players
}

const JoinGameForm: React.FC<JoinGameFormProps> = ({ onJoinSuccess, currentPlayers }) => {
  const [playerName, setPlayerName] = useState('');
  const [playerRole, setPlayerRole] = useState<PlayerRole>('player');
  const [error, setError] = useState<string | null>(null);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // First validate basic naming rules
    const validationError = validateGameName(playerName);
    if (validationError) {
      setError(validationError);
      return;
    ...
    const duplicateError = validatePlayerNameExists(playerName, currentPlayers);
    if (duplicateError) {
